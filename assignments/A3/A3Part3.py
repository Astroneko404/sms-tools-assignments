...
def testRealEven(x):
    ...
    Output:
        The function should return a tuple (isRealEven, dft_buffer, X)
        isRealEven (boolean) = True if the input x is real and even, and False otherwise
        dft_buffer (numpy array, possibly complex) = The M point zero phase windowed version of x
        X (numpy array, possibly complex) = The M point DFT of dft_buffer
    """
    # Your code here
    M = len(x)

    dft_buffer = np.zeros(M)
    half1 = int(math.floor((M+1) / 2))
    half2 = int(math.floor(M / 2))
    dft_buffer[:half1] = x[half2:]
    dft_buffer[M-half2:] = x[:half2]

    X = fft(dft_buffer, M)

    isRealEven = True
    if math.ceil(np.sum(np.abs(np.imag(X)))) > 1e-6:
        isRealEven = False

    return isRealEven, dft_buffer, X
