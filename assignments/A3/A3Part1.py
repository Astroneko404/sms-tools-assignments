...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    # Your code here
    n = len(x)
    p1, p2 = int(fs/f1), int(fs/f2)
    M = int(p1 * p2 / gcd(p1, p2))
    X = fft(x, M)[:int(M / 2 + 1)]
    new_log10 = np.vectorize(log10)
    mX = 20 * new_log10(abs(X))
    return mX


if __name__ == "__main__":
    x = genSine(1.0, 300, 0.0, 48000, 1) + genSine(1.0, 800, 0.0, 48000, 1)
    mX = minimizeEnergySpreadDFT(x, 48000, 300, 800)
    plt.plot(mX)
    plt.show()
